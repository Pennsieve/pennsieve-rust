...
impl FileUpload {
    ...
    ///          the correct entry in the response from the Pennsieve upload
...
impl ETLManifest {
    ...
    #[allow(dead_code)]
    /// Returns a collection of uploaded files, relative to the Pennsieve S3 bucket.
    pub fn files(&self) -> &Vec<String> {
        &self.content.files
    }
}

// See `pennsieve-app/api/src/main/scala/com/pennsieve/uploads/Manifest.scala`
...
impl ManifestEntry {
    ...
    #[allow(dead_code)]
    /// A collection of uploaded files, relative to the Pennsieve S3 bucket.
    pub fn files(&self) -> &Vec<String> {
        &self.manifest.files()
    }
}

/// A preview of a collection of files uploaded to the Pennsieve platform.
#[derive(Clone, Debug, Deserialize, Eq, Hash, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
